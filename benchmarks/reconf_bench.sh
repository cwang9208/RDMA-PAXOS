...
StartDare() {
    for ((i=0; i<${group_size}; ++i)); do
        srv=${servers[$i]}
        config_dare=( "server_type=start" "server_idx=$i" "group_size=$1" "config_path=${DAREDIR}/target/nodes.local.cfg" "dare_log_file=$PWD/srv${i}_1.log" "mgid=$DGID" "LD_PRELOAD=${DAREDIR}/target/interpose.so")
