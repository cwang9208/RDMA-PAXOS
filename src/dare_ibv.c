...
init_one_device( struct ibv_device* ib_dev )
{
 ...
 //       info(log_fp, "# ib_port_attr.lmc = %"PRIu8"\n", ib_port_attr.lmc);
        ...
        uint8_t link_type = ib_port_attr.link_layer;
        info(log_fp, "# link type is: %s\n", link_layer_str(link_type));
        device->gid_index = 0;

        union ibv_gid temp_gid;
        if (ibv_query_gid(device->ib_dev_context, device->port_num, device->gid_index, &temp_gid)) {
            goto error;
        }
        memcpy(device->gid.raw, temp_gid.raw ,16);
